...
class Client {
    ...
    async Answer(yourename, username, message, time, examples, additionally, all_messages) {
    	const [patternanswer, patternsystem] = [patternAnswer, patternSystem].map(text=>
    		...
    		.replace("__current-chat__", all_messages.filter((msg, index, arr)=>(msg.content??"").length > 0 ).map((msg, index, arr)=>(msg.fromMe?`//newmessage// ${msg.content} (${new Date(msg.timestamp*1000).toString()})`:`${(arr[index-1]??msg).fromMe != msg?"\nОтвет аудитории: ": ""}//newmessage// ${msg.content} (${new Date(msg.timestamp*1000).toString()}) ${(arr[index-1]??msg).fromMe != msg.fromMe ?"\n": ""}`)).join(" "))
